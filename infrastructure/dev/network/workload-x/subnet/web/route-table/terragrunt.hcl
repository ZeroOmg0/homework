...
locals {
  name       = "${include.root.locals.project}-${include.root.locals.environment}-workload-x-web-rt"
  tags       = merge(include.root.locals.common_tags, { SubnetType = "web" })
  tgw_routes = include.root.locals.networks.workload_x_web_tgw_routes
}
